...
struct ConverterParams {
     ...
     * numbers.
     * @param   pNumericLocale        specifies whether to honor LC_NUMERIC
     * locale (default true).
     */
 ...
 */
template <>
inline void Converter<std::string>::ToStr(
    const std::string& pVal, std::string& pStr
) const {
    pStr = pVal;
}
 ...
 */
template <>
inline void Converter<std::string>::ToVal(
    const std::string& pStr, std::string& pVal
) const {
    pVal = pStr;
}
...
struct LabelParams {
     ...
     */
    explicit LabelParams(
        const int pColumnNameIdx = 0, const int pRowNameIdx = -1
    ) :
...
struct SeparatorParams {
     ...
     * '\"').
     */
    explicit SeparatorParams(
        const char pSeparator = ',', const bool pTrim = false,
        const bool pHasCR = sPlatformHasCR,
        const bool pQuotedLinebreaks = false, const bool pAutoQuote = true,
...
struct LineReaderParams {
     ...
     * @param   pCommentPrefix        specifies which prefix character to
     * indicate a comment line. Default: #
     * @param   pSkipEmptyLines       specifies whether to skip empty lines.
     * Default: false
     */
    explicit LineReaderParams(
        const bool pSkipCommentLines = false, const char pCommentPrefix = '#',
...
public:
     ...
     * @param   pLineReaderParams     specifies how special line formats should
     * be treated.
     */
    explicit Document(
    ...
    /**
     * @brief   Constructor
     * @param   pStream               specifies a binary input stream to read
     * CSV data from.
     * @param   pLabelParams          specifies which row and column should be
     * treated as labels.
     * @param   pSeparatorParams      specifies which field and row separators
            ...
            if (std::distance(mData.begin(), itRow) >
                ...
                if (dataColumnIdx < itRow->size()) {
                    ...
                    const std::string errStr =
                        ...
                        std::to_string(
                            std::distance(mData.begin(), itRow) -
     ...
     */
    template <typename T>
    std::vector<T> GetColumn(const size_t pColumnIdx, ConvFunc<T> pToVal)
        const {
        const size_t dataColumnIdx = GetDataColumnIndex(pColumnIdx);
        std::vector<T> column;
        for (auto itRow = mData.begin(); itRow != mData.end(); ++itRow) {
     ...
     */
    template <typename T>
    std::vector<T> GetColumn(const std::string& pColumnName, ConvFunc<T> pToVal)
        const {
        const int columnIdx = GetColumnIdx(pColumnName);
        if (columnIdx < 0) {
            throw std::out_of_range("column not found: " + pColumnName);
        ...
        for (auto itRow = pColumn.begin(); itRow != pColumn.end(); ++itRow) {
            ...
            mData
                .at(static_cast<size_t>(
                    std::distance(pColumn.begin(), itRow) +
                    mLabelParams.mColumnNameIdx + 1
                ))
    ...
    template <typename T>
    void SetColumn(
        const std::string& pColumnName, const std::vector<T>& pColumn
    ) {
        const int columnIdx = GetColumnIdx(pColumnName);
        if (columnIdx < 0) {
            throw std::out_of_range("column not found: " + pColumnName);
    ...
    void InsertColumn(
        ...
        if (pColumn.empty()) {
            ...
            Converter<T> converter(mConverterParams);
            for (auto itRow = pColumn.begin(); itRow != pColumn.end();
                 ++itRow) {
                std::string str;
                converter.ToStr(*itRow, str);
                const size_t rowIdx = static_cast<size_t>(
                    std::distance(pColumn.begin(), itRow) +
                    (mLabelParams.mColumnNameIdx + 1)
                );
        ...
        while (column.size() > GetDataRowCount()) {
            std::vector<std::string> row;
            const size_t columnCount = std::max<size_t>(
                static_cast<size_t>(mLabelParams.mColumnNameIdx + 1),
                GetDataColumnCount()
            );
        ...
        for (auto itRow = mData.begin(); itRow != mData.end(); ++itRow) {
            if (std::distance(mData.begin(), itRow) >=
                ...
                itRow->insert(
                    itRow->begin() + static_cast<int>(dataColumnIdx),
                    column.at(rowIdx)
                );
            }
        }
     ...
     */
    template <typename T>
    std::vector<T> GetRow(const std::string& pRowName, ConvFunc<T> pToVal)
        const {
        int rowIdx = GetRowIdx(pRowName);
        if (rowIdx < 0) {
            throw std::out_of_range("row not found: " + pRowName);
    ...
    template <typename T>
    void InsertRow(
        const size_t pRowIdx, const std::vector<T>& pRow = std::vector<T>(),
     ...
     */
    template <typename T>
    T GetCell(const size_t pColumnIdx, const size_t pRowIdx, ConvFunc<T> pToVal)
        const {
        const size_t dataColumnIdx = GetDataColumnIndex(pColumnIdx);
        const size_t dataRowIdx = GetDataRowIndex(pRowIdx);
    ...
    T GetCell(const std::string& pColumnName, const std::string& pRowName)
        ...
        return GetCell<T>(
            static_cast<size_t>(columnIdx), static_cast<size_t>(rowIdx)
        );
    }
    ...
    template <typename T>
    T GetCell(
        const std::string& pColumnName, const std::string& pRowName,
        ConvFunc<T> pToVal
    ) const {
        const int columnIdx = GetColumnIdx(pColumnName);
        if (columnIdx < 0) {
        ...
        return GetCell<T>(
            static_cast<size_t>(columnIdx), static_cast<size_t>(rowIdx), pToVal
        );
    }
    ...
    template <typename T>
    T GetCell(
        const std::string& pColumnName, const size_t pRowIdx, ConvFunc<T> pToVal
    ) const {
        const int columnIdx = GetColumnIdx(pColumnName);
        if (columnIdx < 0) {
            throw std::out_of_range("column not found: " + pColumnName);
    ...
    template <typename T>
    T GetCell(
        const size_t pColumnIdx, const std::string& pRowName, ConvFunc<T> pToVal
    ) const {
        const int rowIdx = GetRowIdx(pRowName);
        if (rowIdx < 0) {
            throw std::out_of_range("row not found: " + pRowName);
    ...
    template <typename T>
    void SetCell(
        const size_t pColumnIdx, const size_t pRowIdx, const T& pCell
    ) {
        const size_t dataColumnIdx = GetDataColumnIndex(pColumnIdx);
        const size_t dataRowIdx = GetDataRowIndex(pRowIdx);
    ...
    void SetCell(
        ...
        SetCell<T>(
            static_cast<size_t>(columnIdx), static_cast<size_t>(rowIdx), pCell
        );
    }
    ...
    template <typename T>
    void SetCell(
        const size_t pColumnIdx, const std::string& pRowName, const T& pCell
    ) {
        const int rowIdx = GetRowIdx(pRowName);
        if (rowIdx < 0) {
            throw std::out_of_range("row not found: " + pRowName);
    ...
    template <typename T>
    void SetCell(
        const std::string& pColumnName, const size_t pRowIdx, const T& pCell
    ) {
        const int columnIdx = GetColumnIdx(pColumnName);
        if (columnIdx < 0) {
            throw std::out_of_range("column not found: " + pColumnName);
...
private:
    ...
    void ReadCsv(std::istream& pStream) {
        ...
        if ((bom2b == bomU16le) || (bom2b == bomU16be)) {
            ...
            wstream.exceptions(
                std::wifstream::failbit | std::wifstream::badbit
            );
            wstream.open(mPath, std::ios::binary);
            if (mIsLE) {
                wstream.imbue(
                    std::
                        locale(wstream.getloc(), new std::codecvt_utf16<wchar_t, 0x10ffff, static_cast<std::codecvt_mode>(std::consume_header | std::little_endian)>)
                );
            } else {
                wstream.imbue(
                    std::
                        locale(wstream.getloc(), new std::codecvt_utf16<wchar_t, 0x10ffff, std::consume_header>)
                );
            }
    ...
    void ParseCsv(std::istream& pStream, std::streamsize p_FileLength) {
        ...
        while (p_FileLength > 0) {
            ...
            // With user-specified istream opened in non-binary mode on windows,
            // we may have a data length mismatch, so ensure we don't parse
            // outside actual data length read.
            ...
            for (size_t i = 0; i < static_cast<size_t>(readLength); ++i) {
                if (buffer[i] == mSeparatorParams.mQuoteChar) {
                    if (cell.empty() ||
                        (cell[0] == mSeparatorParams.mQuoteChar)) {
                        quoted = !quoted;
                    } else if (mSeparatorParams.mTrim) {
                        // allow whitespace before first mQuoteChar
                        const auto firstQuote = std::find(
                            cell.begin(),
                            cell.end(),
                            mSeparatorParams.mQuoteChar
                        );
                        if (std::all_of(cell.begin(), firstQuote, [](int ch) {
                                return isspace(ch);
                            })) {
                            quoted = !quoted;
                        }
                    ...
                    if (mSeparatorParams.mQuotedLinebreaks && quoted) {
                        ...
                        if (mLineReaderParams.mSkipEmptyLines && row.empty() &&
                            ...
                            if (mLineReaderParams.mSkipCommentLines &&
                                !row.at(0).empty() &&
                                (row.at(0)[0] ==
                                 mLineReaderParams.mCommentPrefix)) {
                                // skip comment line
                            } else {
                                mData.push_back(row);
                            }
    ...
    void WriteCsv() const {
        ...
        if (mIsUtf16) {
            ...
            wstream.exceptions(
                std::wofstream::failbit | std::wofstream::badbit
            );
            ...
            if (mIsLE) {
                wstream.imbue(
                    std::
                        locale(wstream.getloc(), new std::codecvt_utf16<wchar_t, 0x10ffff, static_cast<std::codecvt_mode>(std::little_endian)>)
                );
            } else {
                wstream.imbue(
                    std::
                        locale(wstream.getloc(), new std::codecvt_utf16<wchar_t, 0x10ffff>)
                );
            }
        ...
        for (auto itr = mData.begin(); itr != mData.end(); ++itr) {
            for (auto itc = itr->begin(); itc != itr->end(); ++itc) {
                if (mSeparatorParams.mAutoQuote &&
                    ...
                    ReplaceString(
                        str, quoteCharStr, quoteCharStr + quoteCharStr
                    );
    ...
    size_t GetDataColumnCount() const {
        ...
        return (mData.size() > firstDataRow) ? mData.at(firstDataRow).size()
                                             : 0;
    }
    ...
    inline size_t GetDataColumnIndex(const size_t pColumnIdx) const {
        const size_t firstDataColumn = static_cast<size_t>(
            (mLabelParams.mRowNameIdx + 1 >= 0) ? mLabelParams.mRowNameIdx + 1
                                                : 0
        );
    ...
    std::string Trim(const std::string& pStr) const {
        if (mSeparatorParams.mTrim) {
            ...
            str.erase(
                str.begin(),
                std::find_if(
                    str.begin(), str.end(), [](int ch) { return !isspace(ch); }
                )
            );
    ...
    void UpdateRowNames() {
        ...
        if ((mLabelParams.mRowNameIdx >= 0) &&
            ...
            for (auto& dataRow : mData) {
                if (static_cast<int>(dataRow.size()) >
                    mLabelParams.mRowNameIdx) {
                    mRowNames[dataRow[static_cast<size_t>(mLabelParams
                                                              .mRowNameIdx)]] =
                        i++;
                }
            }
...
#pragma warning(pop)
    ...
    static void ReplaceString(
        std::string& pStr, const std::string& pSearch,
